...
- **Basics of applied cryptography**: differential privacy (ε, δ parameters), secure multi-party computation (SMPC), and threat models (honest-but-curious vs. Byzantine)  
...
> **💡 Pro Tip:** Cross-device FL (mobile keyboards, IoT) differs fundamentally from cross-silo FL (hospitals, banks). The former tolerates client dropouts but faces extreme communication constraints; the latter requires Byzantine robustness and strict cryptographic guarantees but assumes reliable connectivity.
...
for each round t = 0 to T-1:
    ...
    w^{t+1} ← Σ_{k∈S_t} (n_k / Σ_{j∈S_t} n_j) · w_k^{t+1}
```
...
> **⚠️ Watch Out:** DP and SecAgg solve different problems. DP protects against membership inference attacks on the *final* model; SecAgg protects against an honest-but-curious *server* seeing raw updates during training. For medical applications, you need both.
...
Upload bandwidth is typically 1000× more constrained than download in mobile networks. Techniques include:  
