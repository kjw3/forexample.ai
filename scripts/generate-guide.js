...
function generateImagePrompt(topic) {
  // Create a concise prompt for Stable Diffusion
  const keywords = topic.tags.slice(0, 3).join(', ');
  return `Modern professional tech illustration about ${topic.title}, ${keywords}, clean design, vibrant gradients with blue purple teal colors, abstract geometric shapes, flowing lines, tech motifs, horizontal landscape, educational, inspiring, high quality digital art`;
}

// Generate and save image using Stable Diffusion via Hugging Face
  ...
  try {
    ...
    if (fs.existsSync(filepath)) {
      ...
      return {
        ...
        credit: 'Generated by Stable Diffusion',
        credit_url: 'https://huggingface.co/stabilityai/stable-diffusion-xl-base-1.0'
      };
    }
    ...
    if (error.response) {
      ...
      if (error.response.status === 503) {
        console.error('Model is loading, please wait and retry...');
      } else {
        console.error('API Response:', error.response.data?.toString('utf-8') || error.response.status);
      }
    }
